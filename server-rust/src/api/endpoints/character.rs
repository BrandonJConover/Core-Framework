...
async fn offline_profile(
    ...
    //
    // The two backend rows (MySqlRow, SqliteRow) are different types so we
    // can't unify the `match` directly. Extract scalars in each arm and
    // unify on a tuple of plain primitives instead.
    let q = "SELECT id, group_id, banned, creation_date FROM players WHERE username = ?";
    let row_opt: Option<(i64, i32, bool, i64)> = match pool {
    ...
    let Some((player_id, group_id, banned, creation_date)) = row_opt else {
        return Ok(None);
    };
