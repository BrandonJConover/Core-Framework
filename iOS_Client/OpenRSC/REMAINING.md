...
- [ ] Terrain projects off-screen: device console shows `[Scene] 370 polys, screen range: x=628..653 y=539..564 (screen=512x334)` — every poly that survives the bbox cull lives past the right edge of the 512×334 buffer, so the terrain mesh never lands on screen even though character billboards (which project from player-local coords) do. The cull at [Scene.swift:387](iOS_Client/OpenRSC/OpenRSC/Sources/Rendering/Scene.swift#L387) checks `poly.minP6 < graphics.width2`, but minP6=628 > 512=width2 should drop the poly — yet 370 such polys survive each frame. Either `World.generateLandscapeModel` is emitting verts in absolute world coords (instead of the player-local frame the camera setup at [RSCGameEngine.swift:289-303](iOS_Client/OpenRSC/OpenRSC/Sources/GameEngine/RSC/RSCGameEngine.swift#L289-L303) assumes) so the camera sits in the wrong half of the mesh, or the cull is comparing the wrong projected fields. Best entry point: temporarily log `model.vertexParam6[0]` and `model.vertXRot[0]` after `scene.endScene` to see whether verts are in tile-local (~0..6144 range for a half-sector) or absolute (~322000+) coords; that pinpoints which side of the bug to fix.
