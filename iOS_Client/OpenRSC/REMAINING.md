...
- [ ] Terrain textures (Phase 5 in progress). Original REMAINING entry assumed a separate `textures.orsc` archive; in reality Java loads textures from Authentic_Sprites.orsc at IDs `spriteTexture(3225)..+EntityHandler.textureCount()` using the "texture" package. Step 1 landed in `1512d7890`: parse bundled texture sprites into Java-style 256-colour dictionaries, retain indexed bytes, and feed expanded ARGB texture pixels into `Scene.loadTexture`. Step 2 landed in `e1c203778`: landscape faces keep their flat colour in `texFront` and record valid terrain texture candidates in `texBack`. Step 3a landed in `4e9112b04`: Scene's current direct terrain fill samples those textures with magenta fallback, giving visible texture content before the full scanline port. Step 3b landed after this note: textured faces now clip sampling to the projected polygon via triangle UVs instead of filling the whole screen-space bounding rectangle. Remaining work: replace the direct sampler with `Shader.shadeScanlineTransparent/Opaque/BlendNormal/Large` and add Java-style lighting/brightness ramps.
