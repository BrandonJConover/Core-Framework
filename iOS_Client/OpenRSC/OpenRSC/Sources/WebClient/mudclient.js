...
// end include: /tmp/tmp28705oz8.js
// include: /tmp/tmpsm7vnfyz.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if ((typeof ENVIRONMENT_IS_WASM_WORKER != 'undefined' && ENVIRONMENT_IS_WASM_WORKER) || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD) || (typeof ENVIRONMENT_IS_AUDIO_WORKLET != 'undefined' && ENVIRONMENT_IS_AUDIO_WORKLET)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: /tmp/tmpsm7vnfyz.js
// include: /tmp/tmp93l2exkk.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach((task) => {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: /tmp/tmp93l2exkk.js
...
var ASM_CONSTS = {
 ...
 187914: ($0, $1, $2, $3, $4) => { var w = $0; var h = $1; var hot_x = $2; var hot_y = $3; var pixels = $4; var canvas = document.createElement("canvas"); canvas.width = w; canvas.height = h; var ctx = canvas.getContext("2d"); var image = ctx.createImageData(w, h); var data = image.data; var src = pixels / 4; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = (val >> 24) & 0xff; src++; dst += 4; } } else { var data32 = new Int32Array(data.buffer); num = data32.length; data32.set(HEAP32.subarray(src, src + num)); } ctx.putImageData(image, 0, 0); var url = hot_x === 0 && hot_y === 0 ? "url(" + canvas.toDataURL() + "), auto" : "url(" + canvas.toDataURL() + ") " + hot_x + " " + hot_y + ", auto"; var urlBuf = _malloc(url.length + 1); stringToUTF8(url, urlBuf, url.length + 1); return urlBuf; },  
 188902: ($0) => { if (Module['canvas']) { Module['canvas'].style['cursor'] = UTF8ToString($0); } },  
 188985: () => { if (Module['canvas']) { Module['canvas'].style['cursor'] = 'none'; } }
};
...
function browser_trigger_keyboard(text,is_password,x,y,width,height,font,is_centred,is_scaled) { const keyboard = is_password ? window._mudclientPassword : window._mudclientKeyboard; keyboard.value = UTF8ToString(text); if (is_centred) { keyboard.style.height = `${height}px`; keyboard.style.textAlign = 'center'; } else { keyboard.style.height = null; keyboard.style.textAlign = 'left'; } keyboard.style.left = `${x}px`; keyboard.style.top = `${y}px`; keyboard.style.width = `${width}px`; if (is_scaled) { keyboard.style.transform = 'scale(2)'; } else { keyboard.style.transform = 'none'; } const fonts = { 1: 'mudclient-font-bold-12', 4: 'mudclient-font-bold-14', 5: 'mudclient-font-bold-16' }; keyboard.classList.remove(...Object.values(fonts)); const fontClass = fonts[font]; if (fontClass) { keyboard.classList.add(fontClass); } keyboard.style.display = 'block'; keyboard.focus(); }
