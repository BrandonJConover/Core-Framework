...
export class SoundTrack {
    ...
    public static load(buffer: Buffer) {
        ...
        // 530-compat: bail if buffer is empty. Original loop reads 16-bit ids until 0xFFFF
        // sentinel; an empty buffer returns 0 forever and would loop the renderer.
        if (!buffer || !buffer.buffer || buffer.buffer.length < 2) return;
        while (buffer.currentPosition + 1 < buffer.buffer.length) {{
