...
export class Region {
    ...
    public static method181(i: number, i_258_: number, is: number[], i_259_: number): boolean {
        ...
        // 530-compat: method181 is a pre-fetch / model-cache audit. The 377
        // implementation pre-walks every loc id and asserts the model is
        // cached on disk before allowing method144 to advance loadingStage
        // 1 → 2. Under a 530 cache we don't have model loading wired yet
        // (idx7 RT5 model parser is its own substantial port), so the
        // strict check would deadlock the world load forever. When a 530
        // cache is detected, we still walk the byte stream (so the buffer
        // position advances and any malformed stream surfaces) but skip
        // the cache check so method144 can return 0 on the next tick.
        const skipModelCheck = !!(globalThis as any).js5Cache;
        let i_260_: number = -1;
        for (;;) {
            {
