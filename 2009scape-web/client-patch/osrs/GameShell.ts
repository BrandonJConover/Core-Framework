...
export class GameShell {
    ...
    public async run() {
        if (!this.runInitialized) {
            ...
            if (this.isMobile) {
                ...
                this.hiddenInput.style.cssText = 'position:fixed;top:-100px;left:0;width:1px;height:1px;opacity:0;';
                document.body.appendChild(this.hiddenInput);

                // Forward hidden input text to game
                this.hiddenInput.addEventListener('input', (e: any) => {
                    const char = e.data;
                    if (char) {
                        const code = char.charCodeAt(0);
                        if (code > 0 && code < 128) {
                            this.keyStatus[code] = 1;
                            this.inputBuffer[this.writeIndex] = code;
                            this.writeIndex = (this.writeIndex + 1) & 127;
                            setTimeout(() => { this.keyStatus[code] = 0; }, 50);
                        }
                    }
                ...
                this.hiddenInput.addEventListener('keydown', (e: KeyboardEvent) => {
                    if (e.key === 'Backspace') {
                        this.keyStatus[8] = 1;
                        this.inputBuffer[this.writeIndex] = 8;
                        this.writeIndex = (this.writeIndex + 1) & 127;
                        setTimeout(() => { this.keyStatus[8] = 0; }, 50);
                        e.preventDefault();
                    } else if (e.key === 'Enter') {
                        this.keyStatus[10] = 1;
                        this.inputBuffer[this.writeIndex] = 10;
                        this.writeIndex = (this.writeIndex + 1) & 127;
                        setTimeout(() => { this.keyStatus[10] = 0; }, 50);
                        e.preventDefault();
                    }
                });
    ...
    private getTouchCanvasCoords(touch: Touch): [number, number] {
        ...
        return [
            ((touch.clientX - rect.left) * scaleX) | 0,
            ((touch.clientY - rect.top) * scaleY) | 0
        ];
    }
