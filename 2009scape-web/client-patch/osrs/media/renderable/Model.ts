...
export class Model extends Renderable {
    ...
    /**
     * 530-cache fallback. When a Game-side preloader populates this map
     * from idx7 via RawModel530.load + Model530.fromRawModel530,
     * getModel() returns those instances first — bypasses the 377
     * OnDemand path entirely so locs / NPCs / players draw without
     * waiting on Jaggrab. Defaults to null; the legacy modelHeaders
     * path runs unchanged when the map is empty.
     */
    public static modelCache530: Map<number, Model> | null = null;

    public static getModel(model: number): Model {
        if (Model.modelCache530) {
            const m530 = Model.modelCache530.get(model);
            if (m530) return m530;
        }
    ...
    public static loaded(id: number): boolean {
        if (Model.modelCache530 && Model.modelCache530.has(id)) return true;
        if (Model.modelHeaders == null) {
            return false;
        }
