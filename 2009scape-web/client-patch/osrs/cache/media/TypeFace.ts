...
export class TypeFace extends Rasterizer {
    ...
    public constructor(large: boolean, archive: Archive, archiveName: string) {
        ...
        // 530-compat: when the archive is a stub, font files aren't present. Synthesize
        // a monospace 1×8 "blank" font so drawString() renders invisibly without crashing.
        ...
        if (!dataFile || !indexFile) {
            ...
            for (let c = 0; c < 256; c++) {
                ...
                this.characterWidths[c] = 1;
                this.characterHeights[c] = 1;
                this.characterXOffsets[c] = 0;
                this.characterYOffsets[c] = 0;
                this.characterScreenWidths[c] = 6; // some width so text layout doesn't divide by 0
            }
            return;
        }
        const dataBuffer: Buffer = new Buffer(dataFile);
        const indexBuffer: Buffer = new Buffer(indexFile);
