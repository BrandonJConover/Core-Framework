...
export class ImageRGB extends Rasterizer {
    ...
    public static fromArchive(archive: Archive, archiveName: string, archiveIndex: number): ImageRGB {
        ...
        const indexBytes = archive.getFile("index.dat");
        // 530-compat: when archive is empty stub, return a 1x1 transparent image.
        if (!imageBytes || !indexBytes) {
            image.width = 1;
            image.height = 1;
            image.maxWidth = 1;
            image.maxHeight = 1;
        ...
        const indexBuffer: Buffer = new Buffer(indexBytes);
