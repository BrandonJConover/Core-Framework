...
export class Widget {
    ...
    public static forId(id: number): Widget {
        if (Widget.interfaces[id] == null) {
            if (Widget.data == null || Widget.data[id] == null) {
                Widget.interfaces[id] = Widget.makeStub(id);
    ...
    // Minimal widget used when a 530 interface ID isn't present in the 377 cache.
    // All array fields are empty (length 0) so callers iterating `children` etc. just no-op.
    private static makeStub(id: number): Widget {
    ...
    public static load(widgetArchive: Archive, fonts: TypeFace[], mediaArchive: Archive) {
        ...
        const data = widgetArchive.getFile("data");
        Widget.mediaArchive = mediaArchive;
        Widget.fonts = fonts;
        // 530-compat: widget "data" isn't in the stubbed archive. Set up empty tables so
        // forId() still returns stubs and calls like Widget.interfaces[j].parentId iterate safely.
        if (!data) {
            Widget.interfaces = [];
            Widget.data = [];
