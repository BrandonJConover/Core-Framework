...
import { LocType530, LocType530Data } from "./LocType530";
...
export class GameObjectDefinition {
    ...
    /**
     * 530 fallback. Populated by Game-side preloader from idx16 via
     * LocType530.load(). When present, getDefinition consults it before the
     * empty 377 loc.dat archive.
     */
    public static cache530: Map<number, LocType530Data> | null = null;
    ...
    public static getDefinition(id: number): GameObjectDefinition {
        ...
        const data530 = GameObjectDefinition.cache530 ? GameObjectDefinition.cache530.get(id) : undefined;
        if (data530) {
            GameObjectDefinition.applyLocType530(definition, data530);
        } else {
    ...
    /**
     * Map a LocType530Data record onto a GameObjectDefinition slot. Field-name
     * translation:
     *   LocType530.models           → modelIds
     *   LocType530.shapes           → modelTypes
     *   LocType530.name             → name
     *   LocType530.width / length   → sizeX / sizeY
     *   LocType530.ops              → options
     *   LocType530.recol_s/recol_d  → originalModelColors/modifiedModelColors
     *   LocType530.resizex/y/z      → modelSizeX/Y/Z
     *   LocType530.xoff/yoff/zoff   → translateX/Y/Z
     *   LocType530.ambient/contrast → modelLightAmbient/modelLightFalloff
     *   LocType530.anim             → animationId
     *   LocType530.mirror           → unknown (mirroring flag)
     *   LocType530.castshadow       → nonFlatShading (proxy — not exact match)
     *   LocType530.adjustToTerrain  ← LocType530.hillskewType !== 0
     *   LocType530.members          → solid (rough mapping)
     *   LocType530.multiLocs        → childrenIds
     *   LocType530.multiLocVarbit   → varbitId
     *   LocType530.multiLocVarp     → configId
     *   LocType530.mapfunction      → icon
     *   LocType530.walloff          → translateY (rough — used for walloff)
     */
    public static applyLocType530(def: GameObjectDefinition, d: LocType530Data) {
        ...
        def.translateX = d.xoff;
        def.translateY = d.yoff;
        def.translateZ = d.zoff;
        def.modelLightAmbient = d.ambient;
        def.modelLightFalloff = d.contrast;
        def.animationId = d.anim;
        def.adjustToTerrain = d.hillskewType !== 0;
        def.childrenIds = d.multiLocs ?? null;
        def.varbitId = d.multiLocVarbit;
        def.configId = d.multiLocVarp;
