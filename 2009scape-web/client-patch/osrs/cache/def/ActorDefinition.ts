...
export class ActorDefinition {
    ...
    public static getDefinition(id: number): ActorDefinition {
        ...
        definition.id = id;
        const data530 = ActorDefinition.cache530 ? ActorDefinition.cache530.get(id) : undefined;
        if (data530) {
            ActorDefinition.applyNpcType530(definition, data530);
        } else {
    ...
    /**
     * Map an NpcType530Data record onto an ActorDefinition slot. Field-name
     * translation:
     *   NpcType530.modelIndices       → modelIds
     *   NpcType530.headmodels         → headModelIndexes
     *   NpcType530.size               → boundaryDimension
     *   NpcType530.combatLevel        → combatLevel
     *   NpcType530.headicon           → headIcon
     *   NpcType530.minimapdisplay     → minimapVisible
     *   NpcType530.recol_s/recol_d    → originalModelColors/modifiedModelColors
     *   NpcType530.ops                → actions[5] (filtered Hidden→null)
     *   NpcType530.contrast           → contrast
     *   NpcType530.ambient            → brightness
     *   NpcType530.multiNpcs          → childrenIds
     *   NpcType530.multiNpcVarbit     → varBitId
     *   NpcType530.multiNpcVarp       → settingId
    ...
    public static applyNpcType530(def: ActorDefinition, d: NpcType530Data) {
        ...
        def.combatLevel = d.combatLevel;
        def.boundaryDimension = d.size;
        def.headIcon = d.headicon;
        def.minimapVisible = d.minimapdisplay;
        def.modelIds = d.modelIndices ?? null;
        def.headModelIndexes = d.headmodels ?? null;
        def.originalModelColors = d.recol_s ?? null;
        def.modifiedModelColors = d.recol_d ?? null;
        def.actions = ActorDefinition.opsToActions(d.ops);
        def.contrast = d.contrast;
        def.brightness = d.ambient;
        def.childrenIds = d.multiNpcs ?? null;
        def.varBitId = d.multiNpcVarbit;
