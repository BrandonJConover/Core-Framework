...
import { ObjType530, ObjType530Data } from "./ObjType530";
...
export class ItemDefinition {
    ...
    /**
     * 530-cache fallback. When populated (by Game preload from idx19 via
     * ObjType530.load), `lookup(id)` consults this map before falling back to
     * the empty 377 archive — gives item tooltips and inventory icons real
     * names + costs + actions instead of the stub fields a 530-cache /
     * 377-parser combo produces.
     */
    public static cache530: Map<number, ObjType530Data> | null = null;
    ...
    public static lookup(id: number): ItemDefinition {
        ...
        definition.id = id;
        definition.reset();
        // 530 path first: if a pre-loaded ObjType530Data exists for this id,
        // map it onto the slot and skip the 377 archive read entirely.
        const data530 = ItemDefinition.cache530 ? ItemDefinition.cache530.get(id) : undefined;
        if (data530) {
            ItemDefinition.applyObjType530(definition, data530);
        } else {
    ...
    /**
     * Map an ObjType530Data record (from rt4 idx19) onto an existing
     * ItemDefinition slot. Field names diverge between the 377 client and
     * the 530 server, so we translate explicitly:
     *   ObjType530.model           → modelId
     *   ObjType530.zoom2d          → modelScale
     *   ObjType530.xAngle2D        → modelRotationX
     *   ObjType530.yAngle2D        → modelRotationY
     *   ObjType530.xOffset2D       → modelOffsetX
     *   ObjType530.yOffset2D       → modelOffsetY
     *   ObjType530.cost            → value
     *   ObjType530.stackable===1   → stackable
     *   ObjType530.ops             → groundActions
     *   ObjType530.iops            → inventoryActions
     *   ObjType530.recol_s/recol_d → originalColours/destColors
     *   ObjType530.certtemplate    → notedTemplateId  (and certlink → notedInfoId)
     *   ObjType530.{man,woman}wear → primary*Model variants
     *   ObjType530.{man,woman}head → primary*HeadPiece variants
     *   ObjType530.resizeX/Y/Z     → groundScaleX/Y/Z
     *   ObjType530.ambient/contrast→ ambience/diffusion
     */
    public static applyObjType530(def: ItemDefinition, d: ObjType530Data) {
        def.name = d.name;
        def.description = d.description ? d.description.split("").map((s) => s.charCodeAt(0)) : null;
        def.modelId = d.model;
        def.modelScale = d.zoom2d;
        def.modelRotationX = d.xAngle2D;
        def.modelRotationY = d.yAngle2D;
        def.modelOffsetX = d.xOffset2D;
        def.modelOffsetY = d.yOffset2D;
        def.value = d.cost;
        def.stackable = d.stackable === 1;
        def.members = d.members;
        def.team = d.team;
        def.groundActions = ItemDefinition.opsToActions(d.ops);
        def.inventoryActions = ItemDefinition.opsToActions(d.iops);
        def.originalColours = d.recol_s as number[] | null;
        def.destColors = d.recol_d as number[] | null;
        def.notedTemplateId = d.certtemplate;
        def.notedInfoId = d.certlink;
        def.primaryMaleModel = d.manwear;
        def.secondaryMaleModel = d.manwear2;
        def.tertiaryMaleEquipmentModel = d.manwear3;
        def.primaryFemaleModel = d.womanwear;
        def.secondaryFemaleModel = d.womanwear2;
        def.tertiaryFemaleEquipmentModel = d.womanwear3;
        def.primaryMaleHeadPiece = d.manhead;
        def.secondaryMaleHeadPiece = d.manhead2;
        def.primaryFemaleHeadPiece = d.womanhead;
        def.secondaryFemaleHeadPiece = d.womanhead2;
        def.maleTranslation = d.manWearXOff;
        def.femaleTranslation = d.womanWearXOff;
        def.groundScaleX = d.resizeX;
        def.groundScaleY = d.resizeY;
        def.groundScaleZ = d.resizeZ;
        def.ambience = d.ambient;
        def.diffusion = d.contrast;
        if (d.countobj && d.countco) {
            def.stackIds = d.countobj;
            def.stackAmounts = d.countco;
        }
    }

    private static opsToActions(ops: (string | null)[] | null | undefined): string[] | null {
        if (!ops) return null;
        const filled = ops.filter((s) => s != null);
        return filled.length > 0 ? (ops as string[]) : null;
    }
