...
export class OnDemandRequester extends Requester implements Runnable{
    ...
    public request(type: number, id: number) {
        ...
        // 530-compat: if the versionlist archive was a stub, there's nothing to fetch.
        // Returning here leaves wanted/wantedToSend empty so immediateRequestsCount()=0
        // and the startup loops complete without an infinite wait.
        if (!this.fileVersions[type] || this.fileVersions[type].length === 0) {
            return;
        }
    ...
    public init(archive: Archive, client: Game) {
        // 530-compat: when the versionlist archive isn't populated, zero out every
        // table so downstream calls to fileCount() / animCount() / model loaders
        // return 0/empty and no on-demand fetch is attempted. The client runs
        // without streaming content (which is fine since we don't render the 3D world).
        const stubEmpty = (archive as any).dataSize === 0;
        if (stubEmpty) {
            for (let v = 0; v < 4; v++) {
                this.fileVersions[v] = [];
                this.filePriorities[v] = [];
                this.fileCrc[v] = [];
            }
            this.modelIndex = [];
            this.regHash = [];
