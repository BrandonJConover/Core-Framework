...
export class Buffer extends CacheableNode {
    ...
    // translated to the 530 equivalent before being written.
    //
    // The server enforces a 20s ping timeout on inbound packets. Mapping the
    // 377 idle-keepalive (168) and heartbeat (40) to 530 PING(93) is what
    // keeps the session alive without sending invalid-format packets.
    // Maps the 377-client outgoing opcode to its 530 equivalent. Payload bodies
    // passthrough unchanged — these are chosen specifically because the byte
    // layout matches between the two revisions.
    //   168 (idle keepalive, 0 body) / 40 (heartbeat, 0 body) → 93 NO_TIMEOUT (0 body)
    //   187 (focus change, 1-byte 1/0)                         → 22 EVENT_APPLET_FOCUS (1-byte 1/0)
    //   202 (long-idle signal, 0 body)                         → 245 AFK_TIMEOUT (0 body)
    // Anything else stays suppressed so the server doesn't hit its 5-invalid-packet kick.
    private static OUTGOING_REMAP: Map<number, number> = new Map<number, number>([
        [168, 93],
        [40,  93],
        [187, 22],
        [202, 245],
    ...
    public getString(): string {
        if (!this.buffer || this.currentPosition >= this.buffer.length) return "";
        const start: number = this.currentPosition;
        while (this.currentPosition < this.buffer.length && this.buffer[this.currentPosition++] !== 10) {}
