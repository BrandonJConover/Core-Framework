...
enum class Decoders530(val opcode: Int) {
    ...
    CONTINUE_OPT(132) {
        override fun decode(player: Player, buffer: IoBuffer): Packet {
            val peek = buffer.toByteBuffer().duplicate()
            val raw = IntArray(Math.min(peek.remaining(), 6))
            for (i in raw.indices) raw[i] = peek.get().toInt() and 0xFF
            if (raw.size >= 4) {
                fun hashInt(b0: Int, b1: Int, b2: Int, b3: Int): Int {
                    return (b0 shl 24) or (b1 shl 16) or (b2 shl 8) or b3
                }
                fun hashLeInt(b0: Int, b1: Int, b2: Int, b3: Int): Int {
                    return b0 or (b1 shl 8) or (b2 shl 16) or (b3 shl 24)
                }
                fun hashIntA(b0: Int, b1: Int, b2: Int, b3: Int): Int {
                    return (b0 shl 8) or b1 or (b2 shl 24) or (b3 shl 16)
                }
                fun hashIntB(b0: Int, b1: Int, b2: Int, b3: Int): Int {
                    return (b0 shl 16) or (b1 shl 24) or b2 or (b3 shl 8)
                }
                fun pairString(label: String, hash: Int): String {
                    val (iface, child) = deHash(hash)
                    return label + "=" + iface + ":" + child
                }
                val slotBe = if (raw.size >= 6) ((raw[4] shl 8) or raw[5]) else -1
                val slotLe = if (raw.size >= 6) (raw[4] or (raw[5] shl 8)) else -1
                val hex = raw.joinToString(" ") { it.toString(16).padStart(2, '0') }
                log(this::class.java, Log.WARN, "Opcode 132 raw=" + hex
                    + " slotBE=" + slotBe + " slotLE=" + slotLe + " candidates "
                    + pairString("int", hashInt(raw[0], raw[1], raw[2], raw[3])) + " "
                    + pairString("leInt", hashLeInt(raw[0], raw[1], raw[2], raw[3])) + " "
                    + pairString("intA", hashIntA(raw[0], raw[1], raw[2], raw[3])) + " "
                    + pairString("intB", hashIntB(raw[0], raw[1], raw[2], raw[3])))
            }
            // RT4 sends continue/dialogue option hashes in intA order. Using intB
            // decodes option interfaces like 232:2 as bogus values such as 512:59392.
            val ifHash = buffer.intA
            val slot = buffer.leShort
            val (iface, button) = deHash(ifHash)
