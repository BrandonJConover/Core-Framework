...
object PacketProcessor {
    ...
    private const val RT4_LAST_MOUSE_X = "rt4:last-mouse-x"
    private const val RT4_LAST_MOUSE_Y = "rt4:last-mouse-y"
        ...
        when (pkt) {
            ...
            is Packet.ReportAbuse -> {
                ...
                core.game.global.report.AbuseReport(
                        pkt.player.name,
                        pkt.target,
                        core.game.global.report.Rule.forId(pkt.rule)
            ...
            is Packet.ContinueOption -> {
                ...
                player.debug("------------------------")
                if (processDirectDialogueAction(player, pkt.iface, pkt.child, -1, pkt.opcode, "continue-option"))
                    return
    ...
    private fun processTrackingPacket(pkt: Packet) {
        when(pkt) {
            ...
            is Packet.TrackingMouseClick -> {
                setAttribute(pkt.player, RT4_LAST_MOUSE_X, pkt.x)
                setAttribute(pkt.player, RT4_LAST_MOUSE_Y, pkt.y)
            }
            else -> {}
        }
    }
    ...
    private fun processIfAction(pkt: Packet.IfAction) {
        ...
        player.debug("-------------------------------------")
        if (processDirectDialogueAction(player, pkt.iface, pkt.child, pkt.optIndex, pkt.opcode, "if-action"))
            return
        if (processTopLevelDialogueAction(pkt))
            return
        if (processInterfaceItemAction(pkt))
            return
    ...
    private fun processInterfaceItemAction(pkt: Packet.IfAction): Boolean {
        if (pkt.slot < 0 || pkt.optIndex < 0)
            return false
        val container = getLikelyContainerForIface(pkt.player, pkt.iface) ?: return false
        if (pkt.slot !in 0 until container.capacity())
            return false
        val item = container[pkt.slot] ?: return false
        if (pkt.itemId >= 0 && item.id != pkt.itemId)
            return false
        processItemAction(Packet.ItemAction(pkt.player, pkt.optIndex, item.id, pkt.slot, pkt.iface, pkt.child))
        return true
    }

    private fun processTopLevelDialogueAction(pkt: Packet.IfAction): Boolean {
        val player = pkt.player
        if (player.dialogueInterpreter.dialogue == null)
            return false
        if (pkt.iface != Components.TOPLEVEL_FULLSCREEN_746 && pkt.iface != Components.TOPLEVEL_548 && pkt.iface != Components.CHATTOP_752)
            return false

        val chatbox = player.interfaceManager.chatbox ?: return false
        if (chatbox.id == InterfaceManager.DEFAULT_CHATBOX)
            return false

        val mouseY = getAttribute(player, RT4_LAST_MOUSE_Y, -1)
        val screenHeight = player.session.clientInfo.screenHeight.takeIf { it > 0 } ?: 503
        val likelyChatboxClick = mouseY < 0 ||
            mouseY >= screenHeight - 190 ||
            pkt.iface == Components.CHATTOP_752 ||
            pkt.child == 44 ||
            pkt.child == 70 ||
            pkt.child == 75
        if (!likelyChatboxClick)
            return false

        val mappedChild = getDialogueButtonId(chatbox.id, mouseY, screenHeight)
        log(
            PacketProcessor::class.java,
            Log.FINE,
            "RT4 wrapper dialogue bridge for ${player.name}: topIface=${pkt.iface}, topChild=${pkt.child}, " +
                "chatbox=${chatbox.id}, mouseY=$mouseY, mappedChild=$mappedChild"
        )
        player.dialogueInterpreter.handle(chatbox.id, mappedChild)
        return true
    }

    private fun processDirectDialogueAction(player: Player, iface: Int, child: Int, optIndex: Int, opcode: Int, source: String): Boolean {
        if (player.dialogueInterpreter.dialogue == null)
            return false

        val chatbox = player.interfaceManager.chatbox ?: return false
        val chatboxId = chatbox.id
        if (chatboxId == InterfaceManager.DEFAULT_CHATBOX || iface != chatboxId)
            return false

        val optionCount = getDialogueOptionCount(chatboxId)
        if (optionCount !in 2..5)
            return false

        val buttonId = normalizeDialogueButtonId(optionCount, child, optIndex) ?: return false
        log(
            PacketProcessor::class.java,
            Log.WARN,
            "RT4 wrapper dialogue direct for ${player.name}: source=$source, opcode=$opcode, " +
                "iface=$iface, child=$child, optIndex=$optIndex, chatbox=$chatboxId, buttonId=$buttonId"
        )
        player.dialogueInterpreter.handle(chatboxId, buttonId)
        return true
    }

    private fun getDialogueOptionCount(chatboxId: Int): Int {
        if (chatboxId !in 228..234 || chatboxId % 2 != 0)
            return -1
        return (chatboxId - 224) / 2
    }

    private fun normalizeDialogueButtonId(optionCount: Int, child: Int, optIndex: Int): Int? {
        if (child in 2..(optionCount + 1))
            return child
        if (child in 1..optionCount)
            return child + 1
        if (optIndex in 0 until optionCount)
            return optIndex + 2
        return null
    }

    private fun getDialogueButtonId(chatboxId: Int, mouseY: Int, screenHeight: Int): Int {
        val optionCount = getDialogueOptionCount(chatboxId)
        if (optionCount in 2..5) {
            if (mouseY < 0)
                return 2
            val chatboxTop = (screenHeight - 165).coerceAtLeast(0)
            val optionTop = chatboxTop + when (optionCount) {
                2 -> 52
                3 -> 45
                4 -> 38
                else -> 32
            }
            val optionBottom = chatboxTop + 142
            val row = (((mouseY - optionTop).coerceAtLeast(0) * optionCount) / (optionBottom - optionTop).coerceAtLeast(1))
    ...
    private fun processNpcAction(pkt: Packet.NpcAction) {
        val rt4NpcDiagnostics = System.getenv("RT4_WEB_NPC_DIAGNOSTICS") == "1"
        if (pkt.npcIndex !in 1 until ServerConstants.MAX_NPCS) {
            if (rt4NpcDiagnostics) {
                log(this::class.java, Log.WARN, "[RT4 WEB NPC] invalid index=${pkt.npcIndex}, opt=${pkt.optIndex}, player=${pkt.player.username}")
            }
            return sendClearMinimap(pkt.player)
        }
        val npc = Repository.npcs[pkt.npcIndex] ?: run {
            if (rt4NpcDiagnostics) {
                log(this::class.java, Log.WARN, "[RT4 WEB NPC] missing npc index=${pkt.npcIndex}, opt=${pkt.optIndex}, player=${pkt.player.username}")
            }
            return sendClearMinimap(pkt.player)
        }
        ...
        if (rt4NpcDiagnostics) {
            log(
                this::class.java,
                Log.WARN,
                "[RT4 WEB NPC] index=${pkt.npcIndex}, opt=${pkt.optIndex}, npc=${npc.id}/${npc.name}, shown=${wrapperChild.id}/${wrapperChild.name}, " +
                    "option=${option?.name ?: "null"}, options=${Arrays.toString(wrapperChild.definition.options)}, " +
                    "playerLoc=${pkt.player.location}, npcLoc=${npc.location}, spawn=${npc.properties.spawnLocation}, " +
                    "tutorialStage=${pkt.player.getAttribute("tutorial:stage", -1)}, tutorialComplete=${pkt.player.getAttribute("tutorial:complete", false)}"
            )
        }
        ...
        val listenerHandled = InteractionListeners.run(wrapperChild.id, IntType.NPC,option.name,pkt.player,npc)
        if (rt4NpcDiagnostics) {
            log(
                this::class.java,
                Log.WARN,
                "[RT4 WEB NPC] listenerHandled=$listenerHandled, interactionLocked=${pkt.player.locks.isInteractionLocked}, " +
                    "movementLocked=${pkt.player.locks.isMovementLocked}, dialogue=${pkt.player.dialogueInterpreter.dialogue?.javaClass?.simpleName ?: "null"}, " +
                    "chatbox=${pkt.player.interfaceManager.chatbox?.id ?: -1}"
            )
        }
        if (listenerHandled)
            return
        val pluginHandled = PluginInteractionManager.handle(pkt.player, wrapperChild, option)
        if (rt4NpcDiagnostics) {
            log(this::class.java, Log.WARN, "[RT4 WEB NPC] pluginHandled=$pluginHandled")
        }
        if (pluginHandled)
            return
        ...
        if (rt4NpcDiagnostics) {
            log(
                this::class.java,
                Log.WARN,
                "[RT4 WEB NPC] default interaction handled, dialogue=${pkt.player.dialogueInterpreter.dialogue?.javaClass?.simpleName ?: "null"}, " +
                    "chatbox=${pkt.player.interfaceManager.chatbox?.id ?: -1}"
            )
        }
    }
    ...
    private fun processItemAction(pkt: Packet.ItemAction) {
        val rt4ItemDiagnostics = System.getenv("RT4_WEB_ITEM_DIAGNOSTICS") == "1"
        val container = getLikelyContainerForIface(pkt.player, pkt.iface) ?: run {
            if (rt4ItemDiagnostics) {
                log(
                    this::class.java,
                    Log.WARN,
                    "[RT4 WEB ITEM] missing container player=${pkt.player.username}, opcodeOpt=${pkt.optIndex}, " +
                        "item=${pkt.itemId}, iface=${pkt.iface}, child=${pkt.child}, slot=${pkt.slot}"
                )
            }
            return
        }
        if (pkt.slot !in 0 until container.capacity()) {
            if (rt4ItemDiagnostics) {
                log(
                    this::class.java,
                    Log.WARN,
                    "[RT4 WEB ITEM] invalid slot player=${pkt.player.username}, opcodeOpt=${pkt.optIndex}, " +
                        "item=${pkt.itemId}, iface=${pkt.iface}, child=${pkt.child}, slot=${pkt.slot}, capacity=${container.capacity()}"
                )
            }
            return
        }
        val item = container.get(pkt.slot) ?: run {
            if (rt4ItemDiagnostics) {
                log(
                    this::class.java,
                    Log.WARN,
                    "[RT4 WEB ITEM] empty slot player=${pkt.player.username}, opcodeOpt=${pkt.optIndex}, " +
                        "item=${pkt.itemId}, iface=${pkt.iface}, child=${pkt.child}, slot=${pkt.slot}"
                )
            }
            return
        ...
        if (item.id != pkt.itemId) {
            if (rt4ItemDiagnostics) {
                log(
                    this::class.java,
                    Log.WARN,
                    "[RT4 WEB ITEM] item mismatch player=${pkt.player.username}, opcodeOpt=${pkt.optIndex}, " +
                        "wanted=${pkt.itemId}, actual=${item.id}, amount=${item.amount}, iface=${pkt.iface}, child=${pkt.child}, slot=${pkt.slot}"
                )
            }
            return
        }
        val option = item.interaction[pkt.optIndex] ?: run {
            if (rt4ItemDiagnostics) {
                log(
                    this::class.java,
                    Log.WARN,
                    "[RT4 WEB ITEM] missing option player=${pkt.player.username}, opcodeOpt=${pkt.optIndex}, " +
                        "item=${item.id}, amount=${item.amount}, iface=${pkt.iface}, child=${pkt.child}, slot=${pkt.slot}, " +
                        "definitionOptions=${Arrays.toString(item.definition.inventoryOptions)}"
                )
            }
            return
        }
        if (pkt.player.locks.isInteractionLocked) {
            if (rt4ItemDiagnostics) {
                log(
                    this::class.java,
                    Log.WARN,
                    "[RT4 WEB ITEM] interaction locked player=${pkt.player.username}, opcodeOpt=${pkt.optIndex}, " +
                        "item=${item.id}, amount=${item.amount}, option=${option.name}, iface=${pkt.iface}, child=${pkt.child}, slot=${pkt.slot}, " +
                        "tutorialStage=${pkt.player.getAttribute("tutorial:stage", -1)}, " +
                        "tutorialComplete=${pkt.player.getAttribute("tutorial:complete", false)}"
                )
            }
            return
        }
        if (rt4ItemDiagnostics) {
            log(
                this::class.java,
                Log.WARN,
                "[RT4 WEB ITEM] handling player=${pkt.player.username}, opcodeOpt=${pkt.optIndex}, item=${item.id}, " +
                    "amount=${item.amount}, option=${option.name}, iface=${pkt.iface}, child=${pkt.child}, slot=${pkt.slot}"
            )
        }
