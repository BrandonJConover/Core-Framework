...
backup-mariadb-full:
	...
	chmod -R 750 $(MYSQL_DUMPS_DIR)
	mkdir -p $(MYSQL_DUMPS_DIR)/full/`date "+%Y%m"`
	chmod -R 750 $(MYSQL_DUMPS_DIR)/full/`date "+%Y%m"`
	@MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysqldump -u${MARIADB_ROOT_USER} "${db}" --single-transaction --quick --lock-tables=false 2> $(MYSQL_DUMPS_DIR)/mysqldump_errors.log | gzip > $(MYSQL_DUMPS_DIR)/full/`date "+%Y%m"`/`date "+%Y%m%d-%H%M-%Z"`-${db}-full.sql.gz
...
backup-mariadb:
	...
	chmod -R 750 $(MYSQL_DUMPS_DIR)/`date "+%Y%m"`
	@if [ "${db}" = "board" ] || [ "${db}" = "wiki" ] || [ "${db}" = "laravel" ]; then \
		MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysqldump -u${MARIADB_ROOT_USER} ${db} --single-transaction --quick --lock-tables=false 2> $(MYSQL_DUMPS_DIR)/mysqldump_errors.log | gzip > $(MYSQL_DUMPS_DIR)/`date "+%Y%m"`/`date "+%Y%m%d-%H%M-%Z"`-${db}.sql.gz; \
	else \
		# Schema-only dump \
		MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysqldump -u${MARIADB_ROOT_USER} ${db} --single-transaction --quick --lock-tables=false --no-data 2> $(MYSQL_DUMPS_DIR)/mysqldump_errors.log > $(MYSQL_DUMPS_DIR)/temp_schema_${db}.sql; \
		# Data-only dump for tables excluding specific log tables \
		MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysqldump -u${MARIADB_ROOT_USER} ${db} --single-transaction --quick --lock-tables=false --no-create-info \
		...
		for table in generic_logs chat_logs trade_logs private_message_logs live_feeds; do \
			MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysqldump -u${MARIADB_ROOT_USER} ${db} $$table --single-transaction --quick --lock-tables=false --no-create-info --where="time >= UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 7 DAY))" 2> $(MYSQL_DUMPS_DIR)/mysqldump_errors.log >> $(MYSQL_DUMPS_DIR)/temp_data_${db}.sql; \
		done; \
		# Data dump for droplogs table with data from last week because droplogs uses different timestamp column name \
		MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysqldump -u${MARIADB_ROOT_USER} ${db} droplogs --single-transaction --quick --lock-tables=false --no-create-info --where="ts >= DATE_SUB(NOW(), INTERVAL 7 DAY)" 2> $(MYSQL_DUMPS_DIR)/mysqldump_errors.log >> $(MYSQL_DUMPS_DIR)/temp_data_${db}.sql; \
...
purge-old-logs:
	@[ "${db}" ] || ( echo ">> db is not set"; exit 1 )
	@MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysql -u${MARIADB_ROOT_USER} "${db}" --execute="DELETE FROM generic_logs WHERE time < UNIX_TIMESTAMP(NOW() - INTERVAL 3 MONTH);"
	@MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysql -u${MARIADB_ROOT_USER} "${db}" --execute="DELETE FROM droplogs WHERE ts < NOW() - INTERVAL 3 MONTH;"
	@MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysql -u${MARIADB_ROOT_USER} "${db}" --execute="DELETE FROM chat_logs WHERE time < UNIX_TIMESTAMP(NOW() - INTERVAL 3 MONTH);"
	@MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysql -u${MARIADB_ROOT_USER} "${db}" --execute="DELETE FROM trade_logs WHERE time < UNIX_TIMESTAMP(NOW() - INTERVAL 3 MONTH);"
	@MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysql -u${MARIADB_ROOT_USER} "${db}" --execute="DELETE FROM private_message_logs WHERE time < UNIX_TIMESTAMP(NOW() - INTERVAL 3 MONTH);"
	@MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysql -u${MARIADB_ROOT_USER} "${db}" --execute="DELETE FROM live_feeds WHERE time < UNIX_TIMESTAMP(NOW() - INTERVAL 3 MONTH);"
...
import-authentic-mariadb:
	@[ "${db}" ] || ( echo ">> db is not set"; exit 1 )
	@MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysql -u${MARIADB_ROOT_USER} "${db}" < server/database/mysql/core.sql
...
import-custom-mariadb:
	@[ "${db}" ] || ( echo ">> db is not set"; exit 1 )
	@MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysql -u${MARIADB_ROOT_USER} "${db}" < server/database/mysql/core.sql
	@MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysql -u${MARIADB_ROOT_USER} "${db}" < server/database/mysql/addons/add_auctionhouse.sql
	@MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysql -u${MARIADB_ROOT_USER} "${db}" < server/database/mysql/addons/add_bank_presets.sql
	@MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysql -u${MARIADB_ROOT_USER} "${db}" < server/database/mysql/addons/add_clans.sql
	@MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysql -u${MARIADB_ROOT_USER} "${db}" < server/database/mysql/addons/add_equipment_tab.sql
	@MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysql -u${MARIADB_ROOT_USER} "${db}" < server/database/mysql/addons/add_harvesting.sql
	@MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysql -u${MARIADB_ROOT_USER} "${db}" < server/database/mysql/addons/add_npc_kill_counting.sql
	@MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysql -u${MARIADB_ROOT_USER} "${db}" < server/database/mysql/addons/add_runecraft.sql
...
import-retro-mariadb:
	@[ "${db}" ] || ( echo ">> db is not set"; exit 1 )
	@MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysql -u${MARIADB_ROOT_USER} "${db}" < server/database/mysql/retro.sql
...
backup-mariadb-local:
	...
	mkdir -p $(MYSQL_DUMPS_DIR)
	chmod -R 750 $(MYSQL_DUMPS_DIR)
	@MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysqldump -u${MARIADB_ROOT_USER} "${db}" --single-transaction --quick --lock-tables=false | zip > $(MYSQL_DUMPS_DIR)/`date "+%Y%m%d-%H%M-%Z"`-${db}.zip
...
restore-mariadb-local:
	...
	@unzip -p $(MYSQL_DUMPS_DIR)/${name} | MYSQL_PWD='${MARIADB_ROOT_PASSWORD}' mysql -u${MARIADB_ROOT_USER} "${db}"
