	...
	public void onUseNpc(Player player, Npc npc, Item item) {
		if (npc.getID() == NpcId.WATCHTOWER_WIZARD.id()) {
			switch (ItemId.getById(item.getCatalogId())) {
				...
				case UNFINISHED_OGRE_POTION -> npcsay(player, npc, "No no, the potion is not complete yet...");
				case OGRE_POTION -> {
				...
				case ARMOUR, WATCH_TOWER_EYE_PATCH, ROBE, DAGGER, GOBLIN_ARMOUR, EYE_PATCH, IRON_DAGGER, WIZARDS_ROBE -> {
					if (player.getQuestStage(Quests.WATCHTOWER) != 1) {
