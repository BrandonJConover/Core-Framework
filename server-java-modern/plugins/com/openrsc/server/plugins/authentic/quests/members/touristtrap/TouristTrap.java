		...
		if (npc.getID() == NpcId.MERCENARY.id()) {
			if (cID == -1) {
				switch (player.getQuestStage(this)) {
					...
					case 6, 7, 8, 9, 10 -> npcsay(player, npc, "Move along now..we've had enough of your sort!");
					case -1 -> npcsay(player, npc, "What're you looking at?");
				}
			}
			switch (cID) {
					...
					if (n != null) {
						if (player.getQuestStage(this) == -1) { // no dialogue after quest on just opening gates
							...
							if (armedVal != Armed.NONE) {
								switch (armedVal) {
									case WEAPON -> npcsay(player, n, "Oi You with the weapon, what are you doing?");
									case ARMOUR -> npcsay(player, n, "Oi You with the armour on, what are you doing?");
									default -> npcsay(player, n, "Oi You with the weapon and armour, what are you doing?");
								}
