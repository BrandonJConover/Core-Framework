	...
	public void onUseLoc(Player player, GameObject obj, Item item) {
		if (obj.getID() == TENT_CHEST_LOCKED && item.getCatalogId() == ItemId.DIGSITE_CHEST_KEY.id()) {
			...
			switch (ItemId.getById(item.getCatalogId())) {
				case BRONZE_PICKAXE -> say(player, null, "I better not - it might break it to pieces!");
				case ROCK_PICK -> say(player, null, "The rockpick is too fat to fit in the gap...");
				case SPADE -> say(player, null, "The spade is far too big to fit");
				case IRON_DAGGER -> say(player, null, "The dagger's blade might break, I need something stronger");
				case BROKEN_ARROW -> say(player, null, "It nearly fits, just a little too thin");
				case TROWEL -> {
			...
			switch (ItemId.getById(item.getCatalogId())) {
				...
				case SPECIMEN_JAR -> say(player, null, "Perhaps not, it might contaminate the samples");
				case JUG -> say(player, null, "I had better not, someone might want to drink from this!");
				case VASE -> say(player, null, "I'm not sure it's good for growing flowers!");
				case EMPTY_VIAL -> {
