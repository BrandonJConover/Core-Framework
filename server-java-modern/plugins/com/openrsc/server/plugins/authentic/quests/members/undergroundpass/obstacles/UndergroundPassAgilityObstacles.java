...
public class UndergroundPassAgilityObstacles implements OpLocTrigger {
	...
	public void onOpLoc(Player player, GameObject obj, String command) {
		if (inArray(obj.getID(), LEDGES)) {
			...
			if (succeed(player, 1)) {
				switch (obj.getID()) {
					case 862 -> player.teleport(730, 3494);
					case 864 -> player.teleport((player.getQuestStage(Quests.UNDERGROUND_PASS) >= 4) || (player.getQuestStage(Quests.UNDERGROUND_PASS) == -1) ? 751 : 734, 3496);
					case 863 -> player.teleport(763, 3442);
					case 872 -> player.teleport((player.getQuestStage(Quests.UNDERGROUND_PASS) >= 4) || (player.getQuestStage(Quests.UNDERGROUND_PASS) == -1) ? 765 : 748, 3497);
					case 865 -> player.teleport(728, 3499);
					case 866 -> player.teleport((player.getQuestStage(Quests.UNDERGROUND_PASS) >= 4) || (player.getQuestStage(Quests.UNDERGROUND_PASS) == -1) ? 755 : 738, 3501);
				}
	...
	private void fallTeleportLocation(Player player, GameObject obj) {
		switch (obj.getID()) {
			case NORTH_STONE_STEP, FIRST_REMAINING_BRIDGE -> player.teleport(738, 584);
			case 898 -> player.teleport(756, 591);
			case 893 -> player.teleport(753, 608);
			case 892 -> player.teleport(734, 596);
			case 896 -> player.teleport(734, 610);
			case 910 -> player.teleport(734, 662);
			case 907 -> player.teleport(733, 646);
			case 906 -> player.teleport(731, 639);
			case 905, 904 -> player.teleport(742, 630);
			case 908 -> player.teleport(760, 638);
			case 909 -> player.teleport(745, 656);
			case 902 -> player.teleport(759, 664);
			case 903 -> player.teleport(761, 613);
			case 901 -> player.teleport(727, 617);
			case 895 -> player.teleport(727, 618);
			case 899 -> player.teleport(734, 619);
			case 900 -> player.teleport(734, 666);
			case 894 -> player.teleport(763, 613);
			case 897 -> player.teleport(753, 585);
		}
	}
}
