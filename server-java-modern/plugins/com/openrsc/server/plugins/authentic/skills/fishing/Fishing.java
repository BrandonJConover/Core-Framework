		...
		if (object.getID() == TUTORIAL_FISH_ID) { // Tutorial Island Shrimp
			...
			for (ObjectFishDef fishDef : fishLst) {
				...
				switch (ItemId.getById(fishDef.getId())) {
					case RAW_BASS -> player.playerServerMessage(MessageType.QUEST, "You catch a bass");
					case RAW_COD -> player.playerServerMessage(MessageType.QUEST, "You catch a cod");
					case RAW_MACKEREL -> player.playerServerMessage(MessageType.QUEST, "You catch a mackerel");
					case OYSTER -> player.playerServerMessage(MessageType.QUEST, "You catch an oyster shell");
					case CASKET -> player.playerServerMessage(MessageType.QUEST, "You catch a casket");
					case BOOTS -> player.playerServerMessage(MessageType.QUEST, "You catch some boots");
					case LEATHER_GLOVES -> player.playerServerMessage(MessageType.QUEST, "You catch some gloves");
					case SEAWEED -> player.playerServerMessage(MessageType.QUEST, "You catch some seaweed");
					default -> player.playerServerMessage(MessageType.QUEST, "You catch something really surprising: a bug! Please report this bug!");
				}
			...
			if (fishLst.size() == 0) {
				...
				switch (ItemId.getById(fish.getCatalogId())) {
					case RAW_SHARK -> player.playerServerMessage(MessageType.QUEST, "You catch a shark!");
					case RAW_SHRIMP -> player.playerServerMessage(MessageType.QUEST, "You catch some shrimps");
					case RAW_ANCHOVIES -> player.playerServerMessage(MessageType.QUEST, "You catch some anchovies");
					default -> {
