...
class DropTest implements Runnable {
	...
	public void run() {
		...
		droppedCount.forEach((key, value) -> {
			...
			if (rate128 > 1) {
				output.append("%,.2f".formatted(rate128)).append(" in 128");
			} else {
				output.append("1 in ").append("%,.1f".formatted((double)finalCount / value));
			}
			output.append(" @whi@ (").append(value).append(" drop%s)".formatted(value == 1 ? "" : "s"));
