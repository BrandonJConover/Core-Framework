...
public final class PlayerModerator implements CommandTrigger {
		...
		if (args.length > 1) {
			try {
				...
				switch (args[1].trim().toLowerCase()) {
					case "head" -> pos = AppearanceId.SLOT_HEAD;
					case "shirt" -> pos = AppearanceId.SLOT_SHIRT;
					case "pants" -> pos = AppearanceId.SLOT_PANTS;
					case "shield" -> pos = AppearanceId.SLOT_SHIELD;
					case "weapon" -> pos = AppearanceId.SLOT_WEAPON;
					case "hat" -> pos = AppearanceId.SLOT_HAT;
					case "body" -> pos = AppearanceId.SLOT_BODY;
					case "legs" -> pos = AppearanceId.SLOT_LEGS;
					case "gloves" -> pos = AppearanceId.SLOT_GLOVES;
					case "boots" -> pos = AppearanceId.SLOT_BOOTS;
					case "amulet" -> pos = AppearanceId.SLOT_AMULET;
					case "cape" -> pos = AppearanceId.SLOT_CAPE;
				}
			}
		}
	...
	private void speakTongues(Player player, int state) {
		...
		switch (state) {
			case 0 -> player.speakTongues = false;
			case 1 -> player.speakTongues = true;
			default -> player.speakTongues = !player.speakTongues;
		}
