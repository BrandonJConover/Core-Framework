...
public final class RegularPlayer implements CommandTrigger {
	...
	private void queryMaxPlayersPerIp(final Player player) {
		player.playerServerMessage(MessageType.QUEST, "%sMax players per ip: %d".formatted(messagePrefix, player.getConfig().MAX_PLAYERS_PER_IP));
		player.playerServerMessage(MessageType.QUEST, "%sYou have %d player(s) logged in.".formatted(messagePrefix, player.getWorld().getServer().getPacketFilter().getPlayersCount(player.getCurrentIP())));
	}
	...
	private void handleQOLOptOut(Player player) {
		if (serverHasQOLEnabled()) {
			...
			if (config().RIGHT_CLICK_BANK) {
				++disablableCount;
				qolExplanation.append("@lre@%d) @whi@Accessing the bank more quickly is disabled.%%".formatted(disablableCount));
			}
			if (config().RIGHT_CLICK_TRADE) {
				++disablableCount;
				qolExplanation.append("@lre@%d) @whi@Right click NPCs to trade with them is disabled.%%".formatted(disablableCount));
			}
			if (config().WANT_BANK_NOTES) {
				++disablableCount;
				qolExplanation.append("@lre@%d) @whi@Ability to withdraw items as bank notes is disabled.%%".formatted(disablableCount));
			}
			if (config().FASTER_YOHNUS) {
				++disablableCount;
				qolExplanation.append("@lre@%d) @whi@Yohnus, who owns the shilo furnace, will not be faster.%%".formatted(disablableCount));
			}
			if (config().WANT_APOTHECARY_QOL) {
				++disablableCount;
				qolExplanation.append("@lre@%d) @whi@The Apothecary will no longer offer to combine vials.%%".formatted(disablableCount));
			}
			if (config().WANT_BETTER_JEWELRY_CRAFTING) {
				++disablableCount;
				qolExplanation.append("@lre@%d) @whi@Jewelry crafting will always show all options.%%".formatted(disablableCount));
			}
	...
	private void handleCertOptOut(Player player) {
		...
		certExplanation.append("@lre@0) @whi@Converting items to certificates is disabled.%%".formatted());

		certExplanation.append("@lre@1) @whi@Trading certificates is disabled.%%".formatted());

		certExplanation.append("@lre@2) @whi@Picking up certificates dropped by other players is disabled.%%".formatted());
	...
	private void queryMinigameLog(Player recipient, String[] args) {
		...
		sb.append("@yel@Minigame Log for %s".formatted(target.getUsername()));
		sb.append(" % %@whi@");
		...
		if (target.getWorld().canYield(new Item(ItemId.BAILING_BUCKET.id()))) {
			...
			sb.append("Fishing Trawler - successful trips: %d%%".formatted(trawlerSuccess));
			sb.append("Fishing Trawler - failed trips: %d%%".formatted(trawlerFailures));
			sb.append(" %");
		}
		...
		if (target.getWorld().canYield(new Item(ItemId.GNOME_BALL.id()))) {
			...
			sb.append("Gnomeball - total goals: %d%%".formatted(gnomeballGoals));
			sb.append(" %");
		}
		...
		if (target.getWorld().canYield(new Item(ItemId.GIANNE_COOK_BOOK.id()))) {
			...
			sb.append("Gnome restaurant - orders delivered: %d%%".formatted(gnomeRestaurantOrders));
			sb.append(" %");
		}
		...
		if (target.getWorld().canYield(new Item(ItemId.GNOME_COCKTAIL_GUIDE.id()))) {
			...
			sb.append("Gnome bar - orders delivered: %d%%".formatted(gnomeBarOrders));
			sb.append(" %");
		}
		...
		if (target.getWorld().canYield(new Item(ItemId.KITTEN.id()))) {
			...
			sb.append("Kittens - raised to adult cats: %d%%".formatted(kittensRaised));
			sb.append("Kittens - released to the wild: %d%%".formatted(kittensReleased));
			sb.append(" %");
		}

		if (target.getConfig().WANT_COMBAT_ODYSSEY) {
			int odysseyCompletions = CombatOdyssey.getPrestige(target);
			sb.append("Odyssey - completions: %d%%".formatted(odysseyCompletions));
			sb.append(" %");
		}
