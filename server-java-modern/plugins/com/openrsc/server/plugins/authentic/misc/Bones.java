...
public class Bones implements OpInvTrigger, UseInvTrigger {
	...
	private void giveBonesExperience(Player player, Item item, boolean bonecrusher) {
		...
		switch (ItemId.getById(item.getCatalogId())) {
			case BONES -> skillXP = 2 * 15; // divided by factor below for 3.75
			case BAT_BONES -> skillXP = 2 * 18; // divided by factor below for 4.5
			case BIG_BONES -> skillXP = 2 * 50; // divided by factor below for 12.5
			case DRAGON_BONES -> skillXP = 2 * 240; // divided by factor below for 60
	...
	private void prayerCape(final Player player, final Item bone) {
		...
		switch (ItemId.getById(bone.getCatalogId())) {
			case BONES, BAT_BONES -> pointsToRestore = 1;
			case BIG_BONES -> pointsToRestore = 2;
			case DRAGON_BONES -> pointsToRestore = 4;
		}
