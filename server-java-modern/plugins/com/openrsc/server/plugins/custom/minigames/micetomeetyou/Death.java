	...
	public void onOpBound(Player player, GameObject obj, Integer click) {
		if (config().MICE_TO_MEET_YOU_EVENT && player.getCache().hasKey("mice_to_meet_you")) {
			...
			switch (questStage) {
				...
				case RECEIVED_EAK, NEED_ASH_TO_ENCHANT -> {
				...
				case EAK_CAN_TALK, AGREED_TO_BRING_BETTY_INGREDIENTS, GIVEN_BETTY_IMMORTAL_MOUSE_INGREDIENTS -> {
					mes("You raise your hand to knock on the door");
										delay(5);
	...
	public void onUseBound(Player player, GameObject gameObject, Item item) {
		if (config().MICE_TO_MEET_YOU_EVENT && player.getCache().hasKey("mice_to_meet_you")) {
			switch (player.getCache().getInt("mice_to_meet_you")) {
				...
				case EAK_HAS_COMPLETED_RECON, EAK_HAS_TOLD_PLAYER_RECON_INFO -> mes("@yel@Eak the Mouse: I was just in there...!");
