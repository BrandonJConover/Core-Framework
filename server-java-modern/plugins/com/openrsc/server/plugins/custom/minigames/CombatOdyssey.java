...
public class CombatOdyssey implements DropObjTrigger, KillNpcTrigger, OpInvTrigger, TalkNpcTrigger,
	...
	public static void radimusDialog(Player player, Npc npc) {
		...
		switch (introStage) {
			...
			case MET_BIGGUM -> npcsay("Hope everything is going well with your quest!");
			case IN_PROGRESS -> {
	...
	private void directToTierMaster(Player player) {
		int currentTier = getCurrentTier(player);
		switch (currentTier) {
			case 0 -> biggumSay(player, "Speak to goblin generals of north Fallington");
			case 1 -> biggumSay(player, "Speak to Thormac the wizzy");
			case 2 -> biggumSay(player, "Speak to ogre Grew of feldip hills");
			case 3, 4 -> biggumSay(player, "Speak to sinister dark mage man in west Ardington");
			case 5, 6 -> biggumSay(player, "Speak to small dumb gnome Hazelmere");
			case 7 -> biggumSay(player, "Speak to Sigbert adventure man");
			case 8 -> biggumSay(player, "Speak to big hero Achetties");
			case 9, 10, 11, 12 -> {
				if (getPrestige(player) < 1) {
			...
			case 13 -> biggumSay(player, "Go claim big shiny reward from Radimus");
		}
	}
	...
	private static void updateCurrentTaskInfo(Player player, int newValue, int save) {
		switch (save) {
			case CURRENT_TIER -> player.getCache().store("combat_odyssey", newValue + ":-1:0");
			case CURRENT_TASK -> player.getCache().store("combat_odyssey", getCurrentTier(player) + ":" + newValue + ":0");
			case CURRENT_KILLS -> player.getCache().store("combat_odyssey", getCurrentTier(player) + ":" + getCurrentTask(player) + ":" + newValue);
		}
	}
