...
public class EakTheMouse implements UsePlayerTrigger, OpInvTrigger, UseNpcTrigger, UseInvTrigger, UseObjTrigger {
		...
		if (config().MICE_TO_MEET_YOU_EVENT && player.getCache().hasKey("mice_to_meet_you")) {
			...
			if (eakCanTalk(player)) {
				switch (questStage) {
					...
					case EAK_HAS_COMPLETED_RECON, EAK_HAS_TOLD_PLAYER_RECON_INFO -> {
					...
					case AGGIE_HAS_GIVEN_PIE, SCARED_DEATH_WITH_EAK -> {
					...
					case DEATH_CONSIDERS_PUMPKIN_PIE_SIDEGIG -> mes("@yel@Eak the Mouse: Let's talk to Death and see if he's made up his mind");
	...
	public void onUseInv(Player player, Integer invIndex, Item item1, Item item2) {
		...
		switch(ItemId.getById(theOtherItem.getCatalogId())) {
			case COINS -> mes("Eak the Mouse: Oh come now " + player.getUsername() + ", ... you don't have to pay me to be your friend!!");
			case BOOTS -> {
				mes("Eak crawls into the boot.");
								delay(3);
								mes("They look really happy inside");
			}
			case CABBAGE -> mes("Eak the Mouse: Gross! You really shouldn't pick that stuff you know.");
			case EGG -> {
			...
			case BRONZE_DAGGER, IRON_DAGGER, STEEL_DAGGER, MITHRIL_DAGGER, ADAMANTITE_DAGGER, RUNE_DAGGER, DRAGON_DAGGER -> {
			...
			case WOOL -> mes("Eak the Mouse: This could make for some lovely bedding. Thankyou");
			case FISH_FOOD, POISONED_FISH_FOOD -> mes("Eak the Mouse: Uhm, I'm not a fish so I think I don't need this...");
			case CHEESE -> {
			...
			case PUMPKIN_PIE, HALF_A_PUMPKIN_PIE -> {
			...
			case WHITE_PUMPKIN_PIE, HALF_A_WHITE_PUMPKIN_PIE -> {
				mes("Eak jumps into the pie");
								delay(3);
								mes("and eats a little bit");
								delay(3);
			...
			default -> mes("Eak the Mouse: wow thanks, but i have no idea what to do with this.");
		}
	}
