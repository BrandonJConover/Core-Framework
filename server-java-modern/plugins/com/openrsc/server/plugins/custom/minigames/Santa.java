...
public class Santa implements TalkNpcTrigger, UseNpcTrigger {
	...
	public void onUseNpc(Player player, Npc npc, Item item) {
		...
		switch (ItemId.getById(item.getCatalogId())) {
			case STAR_COOKIE, CANE_COOKIE, TREE_COOKIE -> {
			...
			case GNOME_WAITER_CHOC_CRUNCHIES, GNOME_WAITER_SPICE_CRUNCHIES, GNOME_WAITER_TOAD_CRUNCHIES, GNOME_WAITER_WORM_CRUNCHIES -> {
				if (player.getCarriedItems().remove(item, true) > -1) {
			...
			case CHOC_CRUNCHIES, SPICE_CRUNCHIES, TOAD_CRUNCHIES, WORM_CRUNCHIES, GNOMECRUNCHIE -> {
				if (player.getCarriedItems().remove(item, true) > -1) {
			...
			case CHOCOLATY_MILK, MILK, GLASS_MILK -> {
				if (player.getCarriedItems().remove(item, true) > -1) {
									npcsay("HO HO HO!!");
			...
			case ASGARNIAN_ALE, BEER, DRAGON_BITTER, DWARVEN_STOUT, GREENMANS_ALE, GROG, WIZARDS_MIND_BOMB, BRANDY, GIN, KARAMJA_RUM, KHALI_BREW, VODKA, WHISKY, DRAYNOR_WHISKY, WINE, ODD_LOOKING_COCKTAIL, BLURBERRY_SPECIAL, CHOCOLATE_SATURDAY, DRUNK_DRAGON, FRUIT_BLAST, PINEAPPLE_PUNCH, SGG, WIZARD_BLIZZARD, BLURBERRY_BARMAN_BLURBERRY_SPECIAL, BLURBERRY_BARMAN_CHOCOLATE_SATURDAY, BLURBERRY_BARMAN_DRUNK_DRAGON, BLURBERRY_BARMAN_FRUIT_BLAST, BLURBERRY_BARMAN_PINEAPPLE_PUNCH, BLURBERRY_BARMAN_SGG, BLURBERRY_BARMAN_WIZARD_BLIZZARD, POISON_CHALICE -> {
			...
			case SANTAS_HAT -> npcsay("That looks just like the hat I lost decades ago...");
			case YOYO -> {
			...
			default -> npcsay("Hoohh... thankyou but I don't need that");
		}
