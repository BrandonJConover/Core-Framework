...
public class PeelingTheOnion implements QuestInterface {
	...
	public static void sedridorDialogue(Player player, Npc npc) {
		...
		switch (questState) {
			...
			case STATE_NOT_BEGUN, STATE_STARTED_QUEST_WITH_KRESH -> {
			...
			case STATE_PLAYER_CONSIDERS_OGRE -> playerReconsidersOgre(player, npc);
			case STATE_SEDRIDOR_SUGGESTED_YOU_VISIT_MAKE_OVER_MAGE, STATE_MAKE_OVER_MAGE_GAVE_WAIVER, STATE_SIGNED_WAIVER -> {
				// menu.add("What was I supposed to do again?");
								npcsay(player, npc, "You should go see the Make over mage about becoming an ogre.");
								say(player, npc, "Right, that makes sense.");
			}
			case STATE_A_NEW_OGRE, STATE_AGGIE_TOLD_PLAYER_TO_COLLECT_ITEMS -> {
	...
	public static void makeOverMageDialogue(Player player, Npc npc) {
		...
		switch (questState) {
			...
			case STATE_A_NEW_OGRE, STATE_AGGIE_TOLD_PLAYER_TO_COLLECT_ITEMS, STATE_AGGIE_HAS_GIVEN_CLAY, STATE_KRESH_NEEDS_RECIPES -> {
			...
			default -> MakeOverMage.makeOverMageAuthenticDialogue(player, npc);
		}
	}
