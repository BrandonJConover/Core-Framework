...
class GameTickEventStore {
    ...
    public boolean add(GameTickEvent event) {
        synchronized (LOCK) {
            ...
            if (event.getOwner() instanceof Player playerOwner) {
                byUsernameHash.put(playerOwner.getUsernameHash(), event);
            } else {
    ...
    public void remove(GameTickEvent event) {
        synchronized (LOCK) {
            ...
            if(event.getOwner() instanceof Player playerOwner) {
                byUsernameHash.remove(playerOwner.getUsernameHash(), event);
            } else {
    ...
    private GameTickKey getKey(GameTickEvent event) {
        if(event instanceof PluginTickEvent pluginTickEvent) {
            return new GameTickKey(pluginTickEvent);
        }
    ...
    class GameTickKey {
        ...
        private String resolveName(GameTickEvent event) {
            if(event instanceof PluginTickEvent pluginTickEvent) {
                return pluginTickEvent.getPluginName();
            }
