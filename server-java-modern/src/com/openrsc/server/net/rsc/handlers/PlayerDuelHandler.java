				...
				for (int i = 0; i < itemCount; i++) {
					...
					if (item.getAmount() < 1) {
						player.setSuspiciousPlayer(true,
							"staking invalid amount of itemId: %d".formatted(item.getCatalogId()));
						continue;
					}
					...
					if (item.getAmount() > (invCount - duelCount)) {
						if (!(player.getConfig().WANT_EQUIPMENT_TAB && item.getAmount() == 1 && player.getCarriedItems().getEquipment().hasEquipped(item.getCatalogId()))) {
							player.setSuspiciousPlayer(true,  "staking insufficient amount of itemId: %d".formatted(item.getCatalogId()));
							player.getDuel().resetAll();
							return;
						}
					}
