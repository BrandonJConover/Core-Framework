			...
			switch (payload.getOpcode()) {
				...
				case SEND_SERVER_CONFIGS:
					...
					for (Object entry : sc.configs) {
						if (entry instanceof Byte byteVal) {
							builder.writeByte(byteVal);
						} else if (entry instanceof String stringVal) {
				...
				case SEND_BANK_PRESET:
					...
					for (Object item : bps.inventoryItems) {
						if (!(item instanceof Item invItem)) {
							builder.writeByte(ItemId.NOTHING.id());
						} else {
							builder.writeShort(invItem.getCatalogId());
							builder.writeByte(invItem.getNoted() ? 1 : 0);
							// amount should only exist if stackable or noted here
							amt = invItem.getAmount();
							if (amt > 0) {
					...
					for (Object item : bps.equipmentItems) {
						if (!(item instanceof Item equipItem)) {
							builder.writeByte(ItemId.NOTHING.id());
						} else {
							builder.writeShort(equipItem.getCatalogId());
							amt = equipItem.getAmount();
							if (amt > 0) {
								builder.writeInt(amt);
							}
						}
				...
				case SEND_UPDATE_PLAYERS:
					...
					for (Object entry : au.info) {
						if (entry instanceof Byte byteVal) {
							builder.writeByte(byteVal);
						} else if (entry instanceof Short shortVal) {
							builder.writeShort(shortVal);
						} else if (entry instanceof Integer intVal) {
							builder.writeInt(intVal);
						} else if (entry instanceof Character charVal) { // wrapper class for appearance byte
							int value = charVal;
