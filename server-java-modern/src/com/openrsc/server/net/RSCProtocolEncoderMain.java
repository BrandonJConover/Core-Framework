		...
		if (!message.isRaw()) {
			...
			if (authenticClient == null || isInauthenticPacket(message.getID()) || authenticClient == -1) {
				...
				if (message.getID() != 191 && message.getID() != 79 && message.getID() != 48) {
					System.out.println("starting to handle opcode %d".formatted(message.getID()));
				}
				*/
				...
				if (packetLength >= 160) {
					...
					if (packetLength != 1) {
						...
						try {
							...
							System.out.println("Warning: index out of bounds on sending last byte of opcode %d".formatted(message.getID()));
