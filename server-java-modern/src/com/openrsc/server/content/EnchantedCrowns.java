...
public class EnchantedCrowns {
	...
	public static void giveBonesExperience(Player player, Item item) {
		...
		int skillXP = switch (ItemId.getById(item.getCatalogId())) {
			case BONES -> 2 * 15; // divided by factor below for 3.75
			case BAT_BONES -> 2 * 18; // divided by factor below for 4.5
			case BIG_BONES -> 2 * 50; // divided by factor below for 12.5
			case DRAGON_BONES -> 2 * 240; // divided by factor below for 60
			default -> {
