...
public class Skills {
	...
	public void addExperience(int skill, int exp) {
		...
		if (completedCycle || achievedMax) {
			...
			if (expCaps[skill] == 0 && getMob() instanceof Player player) {
				expCaps[skill] = System.currentTimeMillis() / 1000;
				try {
					getWorld().getServer().getPlayerService().savePlayerExpCapped(player.getDatabaseID(), skill, expCaps[skill]);
				} catch (GameDatabaseException e) {
					LOGGER.catching(e);
				}
			}
	...
	private void normalize(int skill, boolean sendUpdate) {
		...
		if (sendUpdate)
			sendUpdate(skill);
		if (skill == Skill.PRAYER.id() && getMob() instanceof Player player) {
