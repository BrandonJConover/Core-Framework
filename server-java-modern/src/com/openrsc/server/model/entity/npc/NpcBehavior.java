...
public class NpcBehavior {
		...
		// ran away. It will only change its state after 5 ticks.
			...
			if (canAggro(target) && target == npc.getLastOpponent()) {
				if (target instanceof Player targetPlayer) {
					setChasing(targetPlayer);
				}
				else if (target instanceof Npc targetNpc) {
					setChasing(targetNpc);
				}
	...
	private void handleTackle() {
		...
		if (target instanceof Player targetPlayer) {
			attemptTackle(npc, targetPlayer);
			tackle_retreat();
		}
	}
	...
	private boolean canAggro(final Mob target) {
		...
		boolean lastLogin = target instanceof Player targetPlayer2 && checkCombatTimer(targetPlayer2.getLastLogin(), 5);
		...
		boolean impervious = target instanceof Player targetPlayer3
			&& (targetPlayer3.isInvulnerableTo(npc) || targetPlayer3.isInvisibleTo(npc));
