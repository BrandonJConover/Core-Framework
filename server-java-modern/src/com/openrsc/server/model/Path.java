...
public class Path {
	...
	public void addStep(int x, int y) {
		...
		int maxTiles = 1;
		if(mob instanceof Player player && mob.getConfig().MAX_WALKING_SPEED >= maxTiles) {
		...
		while (Math.abs(diffX) > 0 || Math.abs(diffY) > 0) {
			...
			if (mob.getConfig().PLAYER_BLOCKING == 1) {
				if (mob instanceof Player playerMob && !mob.isFollowing() && pathType == PathType.WALK_TO_POINT) {
					if (Math.abs(diffX) == 1 || Math.abs(diffY) == 1) {
						if (PathValidation.isPlayerBlocking(playerMob, x, y)) {
							return;
						}
					}
				}
