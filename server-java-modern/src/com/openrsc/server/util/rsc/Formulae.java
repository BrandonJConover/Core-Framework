...
public final class Formulae {
	...
	public static int getEmptyJug(int fullJug) {
		return switch (ItemId.getById(fullJug)) {
			case BUCKET_OF_WATER -> ItemId.BUCKET.id();
			case JUG_OF_WATER -> ItemId.JUG.id();
			case BOWL_OF_WATER -> ItemId.BOWL.id();
			default -> ItemId.NOTHING.id();
		};
	}
	...
	public static boolean isP2P(boolean f2pwildy, Object... objs) {
		...
		if (objs.length == 1) {
			...
			if (objs[0] instanceof Integer xCoord && objs[1] instanceof Integer yCoord) {
				x = xCoord;
				y = yCoord;
			}
		}
