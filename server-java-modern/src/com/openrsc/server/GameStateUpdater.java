...
public final class GameStateUpdater {
	...
	protected void updateNpcs(final Player playerToUpdate) {
		...
		if (isRetroClient) {
			// TODO: check impl
			var mobsUpdate = new ArrayList<Object>();
			var clearIdx = new ArrayList<Integer>();
	...
	public void updateNpcAppearances(final Player player) {
		var npcsNeedingHitsUpdate = new ConcurrentLinkedQueue<Damage>();
		var npcMessagesNeedingDisplayed = new ConcurrentLinkedQueue<ChatMessage>();
		var npcProjectilesNeedingDisplayed = new ConcurrentLinkedQueue<Projectile>();
		var npcSkullsNeedingDisplayed = new ConcurrentLinkedQueue<Skull>();
		var npcWieldsNeedingDisplayed = new ConcurrentLinkedQueue<Wield>();
		var npcBubblesNeedingDisplayed = new ConcurrentLinkedQueue<BubbleNpc>();
	...
	public void updatePlayerAppearances(final Player player) {
		var bubblesNeedingDisplayed = new ArrayDeque<Bubble>();
		var chatMessagesNeedingDisplayed = new ArrayDeque<ChatMessage>();
		var projectilesNeedingDisplayed = new ArrayDeque<Projectile>();
		var playersNeedingDamageUpdate = new ArrayDeque<Damage>();
		var playersNeedingHpUpdate = new ArrayDeque<HpUpdate>();
		var playersNeedingAppearanceUpdate = new ArrayDeque<Player>();
	...
	private void issuePlayerAppearanceUpdatePacket(final Player player, final Queue<Bubble> bubblesNeedingDisplayed,
		...
		if (player.loggedIn()) {
			...
			if (updateSize > 0) {
				...
				var updatesMain = new ArrayList<Object>();
				var updatesAlt = new ArrayList<Object>();
				if (isRetroClient) {
	...
	protected void updateGameObjects(final Player playerToUpdate) {
		...
		var struct = new GameObjectsUpdateStruct();
		var objectLocs = new ArrayList<GameObjectLoc>();
		...
		for (var it$ = playerToUpdate.getLocalGameObjects().iterator(); it$.hasNext(); ) {
	...
	protected void updateGroundItems(final Player playerToUpdate) {
		...
		var struct = new GroundItemsUpdateStruct();
		var itemLocs = new ArrayList<ItemLoc>();

		for (var it$ = playerToUpdate.getLocalGroundItems().iterator(); it$.hasNext(); ) {
	...
	protected void updateWallObjects(final Player playerToUpdate) {
		...
		for (var it$ = playerToUpdate.getLocalWallObjects().iterator(); it$.hasNext(); ) {
			final GameObject o = it$.next();
			if (!playerToUpdate.withinGridRange(o) || (o.isRemoved() || o.isInvisibleTo(playerToUpdate))) {
				final int offsetX = o.getX() - playerToUpdate.getX();
				final int offsetY = o.getY() - playerToUpdate.getY();
	...
	protected void sendClearLocations(final Player player) {
		if (player.getLocationsToClear().size() > 0) {
			var struct = new ClearLocationsStruct();
