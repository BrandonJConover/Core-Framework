...
public abstract class GameDatabase {
	...
	public ArrayList<CollectibleItem> getCollectibleItems(final int playerId) throws GameDatabaseException {
		var list = new ArrayList<CollectibleItem>();
		...
		try {
			if (getServer().getConfig().WANT_BANK_PRESETS) {
				...
				for (int k = 0; k < BankPreset.PRESET_COUNT; k++) {
					final ByteArrayOutputStream inventoryBuffer = new ByteArrayOutputStream();
					try (DataOutputStream inventoryWriter = new DataOutputStream(inventoryBuffer)) {
					...
					try (DataOutputStream equipmentWriter = new DataOutputStream(equipmentBuffer)) {
